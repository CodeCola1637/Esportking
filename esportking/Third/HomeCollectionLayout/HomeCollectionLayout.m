...
/** 计算左边缩紧的block，可选实现*/
@property (nonatomic, copy) LeftPaddingBlock paddingBlock;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.paddingBlock != nil) {
        leftPadding = self.paddingBlock(indexPath);
    }
    
    //找出最短行后，计算item位置
    frame.origin = CGPointMake(line * (size.width + self.lineSpacing) + leftPadding, [dict[lineMinHeight] floatValue] + self.collectionSizeHeight);
 ...
 */
- (void)calculateItemSizeWithWidthBlock:(CGSize (^)(NSIndexPath *indexPath))block andLeftPaddingBlock:(LeftPaddingBlock)leftPaddingBlock{
    if (self.block != block) {
        self.block = block;
    }
    if (self.paddingBlock != leftPaddingBlock) {
        self.paddingBlock = leftPaddingBlock;
    }
}
